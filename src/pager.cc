...
void Pager::flush(uint32_t page_num) {
    ...
    if (fs.fail()) {
        std::cerr << "failed to set writing offset: " << offset << std::endl;
        std::exit(EXIT_FAILURE);
    }
    ...
    // 存储 rows 的长度
    auto rows_size = (uint32_t) page->rows.size();
...
std::shared_ptr<Page> Pager::getPage(uint32_t page_num) {
    ...
    if (page == nullptr) {
        ...
        if (page_num <= num_pages && file_len > page_offset) {
            ...
            if (fs.fail()) {
                std::cerr << "failed to set reading offset: " << offset << std::endl;
                std::exit(EXIT_FAILURE);
            }
